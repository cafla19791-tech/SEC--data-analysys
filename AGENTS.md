...
- `scripts/gerar_fluxos.py` + `scripts/resumo_por_agente.py` + `app.py` — BNDES
  cash-flow generator with a per-agent ranking and a **Streamlit** web dashboard
  ("Resumo por Agente Financeiro").
  Branch: `cursor/resumo-por-agente-excel-*` (also `cursor/resumo-agente-financeiro-*`).
...
- The `main` branch is intentionally minimal (only `README.md` and this
  ...
  clean, non-destructive way to run a branch's code without leaving `main` is a
  worktree, e.g. `git worktree add /tmp/app origin/<branch>` and work there.
...
- `pip` installs console scripts (`streamlit`, `pytest`, …) into
  `~/.local/bin`, which is not on `PATH` by default. Invoke via the module form
  to avoid PATH issues: `python3 -m pytest`, `python3 -m streamlit run app.py`.
...
- BNDES generator + Streamlit dashboard (offline sample, verified end-to-end):
  first generate the ranking the UI reads with
  ...
  so the dashboard renders immediately even before running the generator.
