...
Gera::
  ...
  (ou nome customizado com --saida / --titulo)
...
Uso (ContAgil)::
  ...
  python sec_scripts\\apresentacao_impacto_bndes.py --pasta saida\\impacto_diretas ^
    --titulo "Impacto Fiscal — BNDES Diretas" ^
    --saida saida\\APRESENTACAO_IMPACTO_BNDES_DIRETAS.xlsx
"""
...
def _aba_capa(
    ...
    *,
    ...
    ws["A2"] = "Capitalização à data de referência · impacto_fiscal do CSV de fluxos"
    ws["A2"].font = FONT_SUB
    ...
    ws["A15"] = (
        "Fonte: CSV de fluxos (impacto_fiscal) agregado em modo coluna. "
...
def construir_apresentacao(
    ...
    *,
    titulo: str = "Impacto Fiscal — BNDES Indiretas",
) -> Path:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Caminho do xlsx (default: pasta/APRESENTACAO_IMPACTO_BNDES_*.xlsx)",
    )
    p.add_argument(
        "--titulo",
        type=str,
        default="Impacto Fiscal — BNDES Indiretas",
        help="Titulo da aba Capa",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Pasta : {pasta}")
    try:
        out = construir_apresentacao(pasta, args.saida, titulo=args.titulo)
