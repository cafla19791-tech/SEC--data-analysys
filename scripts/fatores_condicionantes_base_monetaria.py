#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fatores condicionantes da base monetária (fim de período), 1995–2025.
...
MARKER = "fatores-base-monetaria-1995-2025-20260906"
...
def carregar_painel(
    ...
    *,
    ...
    inicio: date = date(1994, 12, 1),
    fim: date = date(2025, 12, 31),
...
def fluxo_dezembro(painel: pd.DataFrame, codigo: int, ano: int) -> float:
    ...
    if not alvo:
        # Ano incompleto: último mês disponível
        mes = ultimo_mes_ano(painel, ano)
        if mes is None:
            return float("nan")
        return float(painel.loc[mes, codigo])
...
def anos_com_dados(painel: pd.DataFrame, ano_ini: int = ANO_INICIO, ano_fim: int = ANO_FIM) -> list[int]:
    """Anos civis com pelo menos janeiro (exclui o dezembro-antecessor do estoque)."""
    presentes = sorted({int(y) for y in painel.index.year.unique()})
    com_janeiro = {int(y) for y in painel.index[painel.index.month == 1].year}
    return [a for a in presentes if ano_ini <= a <= ano_fim and a in com_janeiro]
...
def identidade_mensal(painel: pd.DataFrame) -> pd.DataFrame:
    ...
    base = painel[1788]
    linhas = []
    meses = [m for m in painel.index if m >= pd.Timestamp(ANO_INICIO, 1, 1)]
    for mes in meses:
...
def _aba_metodologia(
    ...
    blocos = [
        ...
        (
            ...
            "dezembro de cada ano e mostra o estoque de fim de período. "
            "A variação de 1995 usa o estoque de dezembro/1994.",
        ),
        (
            "Identidade",
...
def _aba_discriminativo(
    ...
    ws["A1"] = (
        "Discriminativo — fatores que influenciam a base monetária (1995–2025)"
    )
    ws["A1"].font = Font(name="Calibri", size=14, bold=True, color=AZUL)
    ...
    # linha de totais 1995–2025 (soma dos fluxos; estoque = último ano)
    tot = 5 + len(disc)
...
def escrever_planilha(
    *,
    ...
    _escrever_matriz(
        ...
        "Fluxo acumulado de janeiro a dezembro. "
        "Estoque da base = último dia de dezembro. Unidade: R$ milhões.",
        anos,
        ultimo,
    )
    ...
    _escrever_matriz(
        ...
        "Fluxo somente do mês de dezembro de cada ano. "
...
def processar(
    *,
    ...
    for a in anos:
        if a not in (1995, 1999, 2002, 2008, 2015, 2020, 2024, 2025):
            continue
        print(f"\n-- {a} --")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--saida",
        type=Path,
        default=ROOT / "output" / "fatores_condicionantes_base_monetaria_1995_2025.xlsx",
    )
