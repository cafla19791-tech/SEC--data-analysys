...
Metodologia (igual ao script de referência):
  ...
  - (opcional) impacto = subsídio × fator_selic(data_impacto) / fator_selic(data_fluxo)
...
Equivalente ContAgil (WinPython), com caminhos típicos:
  pasta_dados   = .../python_jep/winpython/dados
  arquivo_selic = .../python_jep/winpython/STP-....xlsx
...
TAMANHO_LOTE = 50_000
TJLP_TLP_BASE = 0.06
DATA_IMPACTO_DEFAULT = datetime(2026, 6, 30)
...
class SelicSerie:
    ...
    def fator_rapido(self, datas) -> np.ndarray:
        """Mesmo lookup do script ContAgil original (searchsorted side=right)."""
        datas = np.array(datas, dtype="datetime64[ns]")
        idx = np.searchsorted(self.datas, datas, side="right") - 1
        idx[idx < 0] = 0
        return self.fatores[idx]

    def capitalizar(self, valores: np.ndarray, datas_fluxo, data_impacto) -> np.ndarray:
        """Traz valores de datas_fluxo para data_impacto via fator acumulado Selic."""
        f_fluxo = self.fator_rapido(datas_fluxo).astype(float)
        f_impacto = float(self.fator_rapido([np.datetime64(pd.Timestamp(data_impacto), "ns")])[0])
        # evita divisão por zero / fatores inválidos
        f_fluxo = np.where((f_fluxo <= 0) | np.isnan(f_fluxo), np.nan, f_fluxo)
        return valores * (f_impacto / f_fluxo)
...
def gerar_fluxos(
    ...
    data_impacto: datetime | None = None,
) -> tuple[int, list[Path]]:
    """
    Gera fluxos mês a mês (carência + amortização) e grava em lotes CSV.
    Retorna (total_fluxos, caminhos_dos_lotes).

    Se selic_serie e data_impacto forem informados, adiciona coluna
    `impacto` capitalizando o subsídio pelo fator acumulado Selic (STP).
    """
    ...
    usar_impacto = selic_serie is not None and data_impacto is not None

    msg = f"Gerando fluxos (Selic {selic_aa * 100:.2f}% a.a., lote={tamanho_lote:,}"
    if usar_impacto:
        msg += f", impacto em {pd.Timestamp(data_impacto).date()}"
    print(msg + ")...")
    ...
    for row in df.itertuples(index=False):
        ...
        for t in range(carencia + amort):
            ...
            # Igual ao ContAgil: subsidio = saldo * (0.145/12 - taxa)
            ...
            if usar_impacto:
                impacto = float(
                    selic_serie.capitalizar(
                        np.array([subsidio], dtype=float),
                        [np.datetime64(data_fluxo.to_datetime64())],
                        data_impacto,
                    )[0]
                )
                rec["impacto"] = round(impacto, 2) if np.isfinite(impacto) else None
...
def consolidar_resumo(caminhos: list[Path], pasta_saida: Path, stem: str = "fluxos") -> Path | None:
    ...
    sample_cols = pd.read_csv(caminhos[0], nrows=0).columns.tolist()
    usecols = ["contrato", "subsidio", "amortizacao", "data_fluxo", "mes"]
    has_impacto = "impacto" in sample_cols
    if has_impacto:
        usecols.append("impacto")
    ...
    if has_impacto:
        agg["total_impacto"] = ("impacto", "sum")

    resumo = df.groupby("contrato", as_index=False).agg(**agg)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Excel STP da Selic (Bacen). Se informado com --data-impacto, capitaliza o subsídio.",
    )
    p.add_argument(
        "--data-impacto",
        default=None,
        help="Data de referência do impacto fiscal (YYYY-MM-DD). Default: 2026-06-30 se --arquivo-selic.",
    )
    p.add_argument(
        "--selic",
        type=float,
        default=SELIC_AA * 100,
        help="Selic anual %% no cálculo do subsídio nominal (default: 14.5).",
    )
    p.add_argument(
        "--pasta-saida",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.arquivo_selic is not None:
        if not args.arquivo_selic.exists():
            ...
            if args.data_impacto:
                data_impacto = pd.Timestamp(args.data_impacto).to_pydatetime()
            else:
                data_impacto = DATA_IMPACTO_DEFAULT
            print(f"Capitalização Selic até: {pd.Timestamp(data_impacto).date()}")
    ...
    if resumo:
        ...
        if selic_serie is not None:
            tot = pd.read_excel(resumo)
            if "total_impacto" in tot.columns:
                print(f"Soma impacto capitalizado: R$ {tot['total_impacto'].sum():,.2f}")
    return 0
