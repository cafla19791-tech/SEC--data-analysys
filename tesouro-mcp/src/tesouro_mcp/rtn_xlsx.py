"""Parser for Tesouro RTN serie_historica_*.xlsx (ex.: serie_historica_mai26.xlsx)."""

from __future__ import annotations

from collections import defaultdict
from datetime import datetime
from pathlib import Path
from typing import Any

# Rows in sheet 1.1 / 1.1-A (mensal resumida)
ROW_NEEDLES_1_1 = {
    "receita_total": "1. RECEITA TOTAL",
    "despesa_total": "3. DESPESA TOTAL",
    "resultado_primario": "5. RESULTADO PRIMÁRIO GOVERNO CENTRAL - ACIMA DA LINHA",
    "juros_nominais": "9. JUROS NOMINAIS",
    "resultado_nominal": "10. RESULTADO NOMINAL DO GOVERNO CENTRAL",
}

# Sheet 1.5 — transferencias (inclui Fundos Constitucionais agregados)
ROW_NEEDLES_1_5 = {
    "fundos_constitucionais_liquido": "1.2  Fundos Constitucionais",
    "fundos_constitucionais_repasse": "1.2.1  Repasse Total",
}


def _load_workbook(path: str | Path):
    try:
        from openpyxl import load_workbook
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Pacote openpyxl necessario. Instale com: pip install openpyxl"
        ) from exc
    return load_workbook(path, read_only=True, data_only=True)


def list_sheets(path: str | Path) -> dict[str, Any]:
    wb = _load_workbook(path)
    return {
        "path": str(path),
        "sheets": list(wb.sheetnames),
        "note": (
            "Abas *-A = valores constantes IPCA (referencia no titulo da aba, "
            "ex.: Mai/2026). Abas 2.x = anuais (correntes ou % PIB)."
        ),
        "provider": "tesouro-mcp/rtn_xlsx",
    }


def _match_row(label: str, needle: str) -> bool:
    return label.strip().startswith(needle) or needle in label


def _year_of(header: Any) -> int | None:
    if isinstance(header, datetime):
        return header.year
    if isinstance(header, int) and 1900 < header < 2100:
        return header
    return None


def annual_sum_from_monthly_sheet(
    path: str | Path,
    sheet: str,
    needles: dict[str, str],
    *,
    year_from: int = 2001,
    year_to: int = 2025,
    min_months: int = 12,
) -> dict[str, Any]:
    """Sum monthly columns into annual totals (R$ milhoes)."""
    wb = _load_workbook(path)
    if sheet not in wb.sheetnames:
        raise ValueError(f"Aba '{sheet}' nao encontrada. Disponiveis: {wb.sheetnames}")
    ws = wb[sheet]

    headers: tuple[Any, ...] | None = None
    matched: dict[str, tuple[Any, ...]] = {}
    unit = None
    title = None
    for i, row in enumerate(ws.iter_rows(values_only=True), 1):
        if i == 2:
            title = row[0]
        if i == 3 and isinstance(row[0], str):
            unit = row[0]
        if i == 5:
            headers = row
            continue
        lab = row[0]
        if not isinstance(lab, str):
            continue
        for key, needle in needles.items():
            if key not in matched and _match_row(lab, needle):
                matched[key] = row

    if headers is None:
        raise ValueError(f"Cabecalho (linha 5) nao encontrado na aba {sheet}")

    totals: dict[str, dict[int, float]] = {k: defaultdict(float) for k in needles}
    counts: dict[str, dict[int, int]] = {k: defaultdict(int) for k in needles}

    for col, h in enumerate(headers):
        if col == 0:
            continue
        y = _year_of(h)
        if y is None or y < year_from or y > year_to:
            continue
        for key, row in matched.items():
            val = row[col] if col < len(row) else None
            if val is None:
                continue
            try:
                totals[key][y] += float(val)
                counts[key][y] += 1
            except (TypeError, ValueError):
                continue

    series: dict[str, dict[int, float]] = {}
    for key in needles:
        series[key] = {
            y: totals[key][y]
            for y in sorted(totals[key])
            if counts[key][y] >= min_months
        }

    return {
        "path": str(path),
        "sheet": sheet,
        "title": title,
        "unit": unit,
        "labels": {k: matched[k][0] for k in matched},
        "missing": [k for k in needles if k not in matched],
        "series": series,
        "month_counts": {k: dict(counts[k]) for k in counts},
        "provider": "tesouro-mcp/rtn_xlsx",
    }


def extract_annual_rtn(
    path: str | Path,
    *,
    year_from: int = 2001,
    year_to: int = 2025,
    constantes_ipca: bool = False,
    include_fundos: bool = True,
) -> dict[str, Any]:
    """Annual RTN table from serie_historica XLSX.

    If constantes_ipca=True, uses sheet 1.1-A (valores de Mai/AAAA - IPCA).
    Otherwise uses sheet 1.1 (valores correntes).
    """
    sheet = "1.1-A" if constantes_ipca else "1.1"
    core = annual_sum_from_monthly_sheet(
        path,
        sheet,
        ROW_NEEDLES_1_1,
        year_from=year_from,
        year_to=year_to,
    )
    fundos = None
    if include_fundos:
        fundos_sheet = "1.5-A" if constantes_ipca else "1.5"
        try:
            fundos = annual_sum_from_monthly_sheet(
                path,
                fundos_sheet,
                ROW_NEEDLES_1_5,
                year_from=max(year_from, 2008),
                year_to=year_to,
            )
        except Exception as exc:  # noqa: BLE001
            fundos = {"error": str(exc)}

    years = sorted(
        set().union(*[set(v.keys()) for v in core["series"].values()])
    )
    rows = []
    for y in years:
        row = {"ano": y}
        for key, series in core["series"].items():
            mi = series.get(y)
            row[f"{key}_R$mi"] = None if mi is None else round(mi, 2)
            row[f"{key}_R$bi"] = None if mi is None else round(mi / 1000.0, 2)
        if isinstance(fundos, dict) and "series" in fundos:
            for key, series in fundos["series"].items():
                mi = series.get(y)
                row[f"{key}_R$mi"] = None if mi is None else round(mi, 2)
                row[f"{key}_R$bi"] = None if mi is None else round(mi / 1000.0, 2)
        rows.append(row)

    return {
        "path": str(path),
        "sheet": sheet,
        "constantes_ipca": constantes_ipca,
        "unit": core.get("unit"),
        "year_from": year_from,
        "year_to": year_to,
        "count": len(rows),
        "labels": core.get("labels"),
        "fundos_note": (
            "Fundos Constitucionais na aba 1.5 sao transferencias da Uniao "
            "(agregado FNO/FNE/FCO no RTN), nao desembolsos por banco."
        ),
        "rows": rows,
        "provider": "tesouro-mcp/rtn_xlsx",
    }
