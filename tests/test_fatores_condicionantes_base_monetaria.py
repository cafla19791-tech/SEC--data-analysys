...
def test_carregar_painel_e_planilha(tmp_path: Path):
    ...
    cel_neg = None
    for row in ws.iter_rows(min_row=5, max_col=1 + idx_tit):
        if str(row[0].value) == "2000":
            cel_neg = row[idx_tit]
            break
    assert cel_neg is not None
    ...
    # 2000 e 2001 presentes; anos fora da amostra sintética ausentes
