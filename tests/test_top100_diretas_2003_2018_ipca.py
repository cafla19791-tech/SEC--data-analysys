...
def test_periodo_2019_2022(tmp_path: Path):
    """Garante filtro de anos 2019–2022 no ranking."""
    ...
    ws.append(["IN", "11111111000111", datetime(2018, 12, 31), 9999, 9999, "DIRETA"])
    ws.append(["OK", "22222222000122", datetime(2019, 1, 1), 100, 100, "DIRETA"])
    ws.append(["OK2", "33333333000133", datetime(2022, 12, 31), 200, 200, "DIRETA"])
    ws.append(["OUT", "44444444000144", datetime(2023, 1, 1), 9999, 9999, "DIRETA"])
    wb.create_sheet("DISCLAIMER")
    wb.create_sheet("DE-PARA CNAE")
    wb.save(fonte)
    ...
    assert set(ranking[COL_NOME]) == {"OK", "OK2"}
    capa = {
        r[0]: r[1]
        for r in load_workbook(saida, read_only=True)["Capa"].iter_rows(values_only=True)
        if r[0]
    }
    assert capa["Período"] == "2019–2022"
